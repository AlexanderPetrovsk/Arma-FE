...
function DatePicker(props) {
    ...
    useClickOutside(datepickerContainerRef, () => { setShowDatePicker(false) }, showDatePicker);


    const datepickerPopupRef = useRef(null);

    function handleDatepickerOutOfBounds () {
        const rect = datepickerPopupRef.current.getBoundingClientRect();

        if (rect.right > (window.innerWidth || document.documentElement.clientWidth)) {
            datepickerPopupRef.current.style.left = `${window.outerWidth - rect.width - 10}px`;
        }
    }

    useEffect(() => {
        if (datepickerPopupRef.current) {
            handleDatepickerOutOfBounds();
        }
    }, [showDatePicker]);

    const datePickerPopup = () => {
        return (
            <div className='datepicker-popup' ref={datepickerPopupRef}>
    ...
    return (
        <div className={`datepicker-container ${props.classes ?? ''}`} ref={datepickerContainerRef}>
            <input className='datepicker-toggle text-input' placeholder={props.placeholder ?? 'Select date'} value={selectedDate} onClick={() => handleDatePickerClick()} readOnly />
